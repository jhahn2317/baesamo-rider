      ...
      {selectedShiftDetail && (
         <div className="fixed inset-0 bg-black/70 flex items-end justify-center z-[80] p-0">
            <div className="bg-white w-full max-w-md rounded-t-[3rem] p-6 pb-12 shadow-2xl relative border-t-8 border-blue-600 animate-in slide-in-from-bottom duration-300">
               ...
               {/* 💡 [복구 확인] 읽기 전용일 때는 닫기 버튼만 노출, 아닐 때는 수정/삭제 버튼 노출 */}
               {isReadOnly ? (
...
export default function App() {
  ...
  // 💡 커서 AI 오류 해결: 리스너 클린업(Cleanup) 완벽 적용
  useEffect(() => {
    let unsubUser = null;
    ...
    const unsubAuth = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        unsubAllUsers = onSnapshot(query(collection(db, 'users'), where('status', 'in', ['approved', 'admin'])), (s) => 
          setAllUsers(s.docs.map(doc => ({ ...doc.data(), uid: doc.id })))
        );
        unsubNotice = onSnapshot(doc(db, 'settings', 'globalNotice'), (s) => 
  ...
  // 💡 커서 AI 경고 해결: return 값 명시
  useEffect(() => { 
    if (isAdmin) { 
      const q = query(collection(db, 'users'), where('status', '==', 'pending')); 
      const unsubPending = onSnapshot(q, (s) => setPendingUsers(s.docs.map(doc => ({ ...doc.data(), uid: doc.id })))); 
      return () => unsubPending();
    } 
  }, [isAdmin]);
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-slate-50 overflow-hidden font-sans select-none">
      ...
      {/* 💡 가면 쓴 상태에서는 헷갈리지 않게 하단 네비게이션 숨기기 */}
      {!impersonatingUser && (
